...
export class ComplianceController {
  ...
  async buildExecution(
    ...
    const data = await this.executionService.buildExecution(
      inventoryId,
      templateId,
      periodKey,
  ...
  @UseInterceptors(
    AnyFilesInterceptor({
      ...
      fileFilter: (_request, file, callback) => {
        if (!CHECKLIST_PHOTO_MIMES.includes(file.mimetype)) {
          return callback(new Error('Foto checklist harus JPEG, PNG, atau WebP'), false);
        }
        callback(null, true);
      },
  ...
  async submit(
    ...
    const answers = dto.answers.map(answer => ({
      ...answer,
      photo: photoByQuestion.get(answer.questionId),
    }));
    const data = await this.executionService.submit(
      inventoryId,
      templateId,
      dto.periodKey,
      dto.sessionId ?? null,
