...
export class ComplianceEvidenceService {
  ...
  async list(query: { search?: string; month?: string; page?: number; limit?: number }) {
    ...
    if (query.month && /^\d{4}-\d{2}$/.test(query.month)) {
      where.checklistLog = { periodKey: { startsWith: query.month } };
    }
    ...
    const [total, rows, findings, inventoryRows, uploaderRows] = await Promise.all([
      ...
      this.prisma.evidence.findMany({
        ...
        include: {
          inventory: { include: { itemType: true, area: true } },
          checklistLog: { include: { question: true, template: true } },
      ...
      this.prisma.evidence.count({
        where: {
          ...where,
          checklistLog: { ...(where.checklistLog || {}), status: 'not_ok' },
        },
      }),
      this.prisma.evidence.findMany({
        where,
    ...
    return {
      items: rows.map((row) => ({
        ...
        url: `/api/v1/compliance/evidence/${row.id}/file`,
        inventory: {
          id: row.inventory.id,
          assetCode: row.inventory.assetCode,
          itemType: row.inventory.itemType?.name ?? null,
          area: row.inventory.area?.name ?? null,
          specificArea: row.inventory.specificArea,
        },
        checklist: {
          logId: row.checklistLogId,
          template: row.checklistLog.template.name,
          question: row.checklistLog.question?.questionText ?? null,
          periodKey: row.checklistLog.periodKey,
          periodLabel: row.checklistLog.periodLabel,
          status: row.checklistLog.status,
        },
  ...
  async persistUploadedFiles(input: {
    inventoryId: number;
    templateId: number;
    periodKey: string;
    ...
    const questionIds = files
      .map((file) => Number(/^photo_(\d+)$/.exec(file.fieldname)?.[1]))
  ...
  async getFilePath(evidenceId: number) {
    ...
    try {
      ...
      throw new NotFoundException('File evidence tidak ditemukan');
    }
