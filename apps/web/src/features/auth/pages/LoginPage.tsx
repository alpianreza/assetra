...
const loginSchema = z.object({ identifier: z.string().min(1, 'Username atau email wajib diisi'), password: z.string().min(1, 'Password wajib diisi') });
...
export function LoginPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: Error) => setServerError(error.message || 'Terjadi kesalahan saat login'),
  });
  ...
  return <div className="relative min-h-screen overflow-hidden bg-slate-950 p-0 lg:grid lg:place-items-center lg:p-6">
    ...
    <div className="absolute right-4 top-4 z-20 rounded-full border border-white/10 bg-slate-950/50 p-1 text-white backdrop-blur"><ThemeToggle /></div>
    ...
    <main className="relative z-10 mx-auto grid min-h-screen w-full max-w-5xl overflow-hidden bg-card shadow-2xl lg:min-h-[620px] lg:grid-cols-[1.08fr_0.92fr] lg:rounded-3xl lg:border lg:border-white/10">
      <section className="relative hidden overflow-hidden bg-[linear-gradient(145deg,#101d35,#15294b_58%,#123650)] p-12 text-white lg:flex lg:flex-col lg:justify-between"><div className="absolute -bottom-32 -right-28 h-80 w-80 rounded-full border border-white/10 shadow-[0_0_0_48px_rgba(255,255,255,0.025),0_0_0_96px_rgba(255,255,255,0.015)]" /><div className="relative"><div className="mb-12 flex items-center gap-3">{branding?.logoUrl ? <img src={branding.logoUrl} alt={companyName} className="h-14 max-w-32 object-contain" /> : <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 via-violet-500 to-cyan-400"><span className="h-5 w-5 rounded-full bg-[#132541]" /></span>}<div><p className="text-xl font-bold">{branding?.shortName || companyName}</p><p className="text-[10px] uppercase tracking-[0.2em] text-white/55">Enterprise workspace</p></div></div><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-cyan-300"><ShieldCheck className="h-4 w-4" />Asset & compliance management</p><h1 className="mt-4 max-w-md text-4xl font-bold leading-tight tracking-[-0.04em]">Kelola aset dan kepatuhan dengan lebih terarah.</h1><p className="mt-4 max-w-md text-sm leading-7 text-white/65">Assetra menyatukan checklist, inventaris, QR, temuan, dan laporan operasional dalam satu sistem.</p><div className="mt-10 space-y-4"><Benefit icon={CheckSquare2} text="Checklist terjadwal dan mudah dipantau" /><Benefit icon={BarChart3} text="Analitik progres secara real-time" /><Benefit icon={LockKeyhole} text="Akses aman berdasarkan peran pengguna" /></div></div><p className="relative text-xs text-white/40">© {new Date().getFullYear()} {companyName}</p></section>

      <section className="flex min-h-screen items-center bg-card px-6 py-20 sm:px-12 lg:min-h-0 lg:px-14 lg:py-12"><div className="mx-auto w-full max-w-sm"><div className="mb-9 lg:hidden">{branding?.logoUrl ? <img src={branding.logoUrl} alt={companyName} className="mb-4 h-14 max-w-32 object-contain" /> : <span className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-xl font-bold text-primary-foreground">A</span>}<p className="font-bold">{branding?.shortName || companyName}</p></div><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Enterprise Asset Management</p><h2 className="mt-2 text-3xl font-bold tracking-tight">Masuk ke Assetra</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Gunakan akun perusahaan untuk melanjutkan ke workspace Anda.</p>
        <form onSubmit={handleSubmit(values => { setServerError(null); mutation.mutate(values); })} className="mt-8 space-y-5"><div className="space-y-2"><Label htmlFor="identifier">Username atau Email</Label><div className="relative"><UserRound className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="identifier" autoFocus autoComplete="username" placeholder="Masukkan username atau email" className="h-12 pl-10" {...register('identifier')} /></div>{errors.identifier && <p className="text-xs text-destructive">{errors.identifier.message}</p>}</div><div className="space-y-2"><Label htmlFor="password">Password</Label><div className="relative"><LockKeyhole className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Masukkan password" className="h-12 px-10" {...register('password')} /><button type="button" onClick={() => setShowPassword(value => !value)} className="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={showPassword ? 'Sembunyikan password' : 'Tampilkan password'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div>{errors.password && <p className="text-xs text-destructive">{errors.password.message}</p>}</div>{serverError && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Login gagal</AlertTitle><AlertDescription>{serverError}</AlertDescription></Alert>}<Button type="submit" className="h-12 w-full" disabled={mutation.isPending}>{mutation.isPending ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />Memproses...</> : <>Masuk<ArrowRight className="ml-2 h-4 w-4" /></>}</Button></form><p className="mt-5 flex items-center justify-center gap-2 text-xs text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5" />Koneksi aman untuk pengguna terotorisasi</p></div></section>
    </main>
  </div>;
}
