...
export async function fetchDashboardSummary(params: { areaId?: string; categoryId?: string }): Promise<DashboardResponse> {
...
export async function exportComplianceXlsx(payload: ExportPayload): Promise<Blob> {
  ...
  if (!res.ok) {
    throw new Error(`Compliance export failed: ${res.status}`)
  }
  return res.blob()
}
...
export async function exportInventoryXlsx(): Promise<Blob> {
  ...
  if (!res.ok) {
    throw new Error(`Inventory export failed: ${res.status}`)
  }
  return res.blob()
}
