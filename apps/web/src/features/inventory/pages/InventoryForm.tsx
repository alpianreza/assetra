...
import { z } from 'zod';
import { useInventoryDetail, useCreateInventory, useUpdateInventory, useNextAssetCode } from '../hooks';
import { useAreas } from '../../master-data/hooks';
import { useItemTypes } from '../../master-data/hooks';
import { useUsers } from '../../users/hooks';

// Nomor inventaris is generated by the server (KODEKATEGORI-KODEITEM-NOURUT),
// so it is not part of the form payload.
...
export function InventoryForm() {
  ...
  // Must stay below useForm: `watch` is not initialised before this point.
  const watchedItemTypeId = Number(watch('itemTypeId')) || 0;
  const selectedItemType = itemTypes.find((t: any) => t.id === watchedItemTypeId);

  // Preview the code only when creating; an existing code never changes.
  const { data: previewData, isFetching: isPreviewing } = useNextAssetCode(
    isEdit ? undefined : watchedItemTypeId,
  );

  const assetCodePreview = isEdit
    ? inventoryData?.data?.assetCode ?? ''
    : previewData?.data?.assetCode ?? '';
  ...
  const togglePic = (userId: number) => {
    ...
    setValue(
      ...
      current.includes(userId)
        ? current.filter((picId) => picId !== userId)
        : [...current, userId]
    );
  };
      ...
      <form onSubmit={handleSubmit(handleSubmitForm)} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            value={
              assetCodePreview ||
              (isPreviewing ? 'Menyiapkan...' : 'Otomatis setelah jenis item dipilih')
            }
            className="w-full border rounded-lg p-2 mt-1 bg-muted/50 text-muted-foreground"
          />
          <p className="text-xs text-muted-foreground mt-1">
            {isEdit
              ? 'Nomor inventaris tidak dapat diubah setelah dibuat.'
              : 'Dibuat otomatis dari kode kategori, kode jenis item, dan nomor urut.'}
          </p>
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground">Keterangan Tipe</label>
          <input
