...
import { Activity, Boxes, CheckCircle2, CircleAlert, Clock3, MapPin, ShieldCheck, Tags, Wrench, type LucideIcon } from 'lucide-react';
...
function MetricSwiper({ items }: { items: SlideMetric[] }) {
  ...
  return <section className="overflow-hidden" aria-label="Ringkasan analitik bergerak">
    <style>{`
    ...
    <div className="assetra-metric-track flex w-max gap-[18px] py-1">
      {slides.map(({ label, value, note, icon: Icon, tone, text, iconTone, href }, index) => <Link key={`${label}-${index}`} to={href} className="block min-w-[var(--metric-card-width)]" aria-hidden={index >= items.length} tabIndex={index >= items.length ? -1 : 0}>
        <article className={`w-full rounded-lg border-0 p-6 shadow-none ${tone}`}>
          <div className="text-center transition-transform duration-300 ease-in-out hover:scale-105">
            <div className="mb-3 flex justify-center"><span className={`flex h-[50px] w-[50px] items-center justify-center rounded-2xl ${iconTone}`}><Icon className="h-7 w-7" /></span></div>
